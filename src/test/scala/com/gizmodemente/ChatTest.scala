...
class ChatTest extends TestKit(ActorSystem("chat-spec")) with ImplicitSender with WordSpecLike
  ...
  "A supervisor" must {
    ...
    "Create a user that create a chat and send message to this chat" in {
      ...
      val userActorPath = Await.result(userActorFuture.mapTo[String], timeout.duration)
      val userActor = system.actorSelection(userActorPath)
      userActor ! RequestNewChat("Test Chat")
      Thread.sleep(1000)
      val userActorFuture2 = supervisor ? CreateUser("TestUser2")
      val userActor2path = Await.result(userActorFuture2.mapTo[String], timeout.duration)
      val userActor2 = system.actorSelection(userActor2path)
      Thread.sleep(1000)
